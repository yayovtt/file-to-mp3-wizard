...
export const transcribeAudioChunked = async (
  ...
  // Split file if necessary (using 25MB limit to be very safe)
  const chunks = await splitAudioFile(audioFile, 25 * 1024 * 1024);
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    // Verify chunk size before sending - now with stricter limit
    if (chunk.blob.size > 30 * 1024 * 1024) { // 30MB as absolute max
